...
function barChart(
  ...
  for (const d of data) {
    ...
    const valStr = d.value >= 1000 ? formatNum(d.value) : d.value.toFixed(precision)
...
output.push(
  `> Benchmarked on ${new Date().toISOString().split('T')[0]} — Deno ${Deno.version.deno}, ${Deno.build.arch}-${Deno.build.os}\n`,
)
...
for (const n of scaleSizes) {
  ...
  scaleResults.push(r)
  console.log(
    `    insert: ${formatNum(r.insertOpsPerSec)}/s  search: ${r.searchMedianUs.toFixed(0)}µs  recall@ef200: ${
      (r.recall200 * 100).toFixed(1)
    }%  recall@ef500: ${(r.recall500 * 100).toFixed(1)}%  mem: ${r.memoryMB.toFixed(1)}MB`,
  )
}
...
output.push(table(
  ...
  scaleResults.map((r) => [
    ...
    r.insertTimeMs < 1000 ? `${r.insertTimeMs.toFixed(0)}ms` : `${(r.insertTimeMs / 1000).toFixed(1)}s`,
...
console.log('\n▸ Recall vs latency tradeoff (10K, 128d)...')
{
  ...
  for (const ef of [10, 50, 100, 200, 400]) {
    ...
    console.log(
      `  ef=${ef}: recall=${recall.toFixed(1)}%  latency=${sl.medianUs.toFixed(0)}µs`,
    )
  }
  ...
  output.push(
    `         ${efRows[0].medianUs.toFixed(0)}µs${' '.repeat(chartW - 12)}${efRows[efRows.length - 1].medianUs.toFixed(0)}µs`,
  )
...
console.log('\n▸ Dimension scaling (5K vectors)...')
{
  ...
  for (const d of dimSizes) {
    ...
    console.log(
      `    insert ${formatNum((n / insertMs) * 1000)}/s  search ${formatNum(1_000_000 / sl.medianUs)}/s  mem ${memShrunkMB.toFixed(1)}MB`,
    )
  }
  ...
  for (const r of dimRows) {
    output.push(`<details>`)
    output.push(
      `<summary><b>${r.dim} dimensions</b> — ${formatNum(r.searchOps)} search/s, ${r.memShrunkMB.toFixed(1)} MB</summary>\n`,
    )
    output.push(table(
      ['Metric', 'Value'],
      [
        ...
        [
          'Build time',
          r.buildTime < 1000 ? `${r.buildTime.toFixed(0)}ms` : `${(r.buildTime / 1000).toFixed(1)}s`,
        ],
...
console.log('\n▸ Serialization throughput...')
{
  ...
  for (const n of [1_000, 10_000, 25_000]) {
    ...
    console.log(
      `  ${formatNum(n)}: ${sizeMB.toFixed(1)} MB  enc ${encMs.toFixed(0)}ms  dec ${decMs.toFixed(0)}ms`,
    )
  }
...
console.log('\n▸ M parameter impact (10K, 128d)...')
{
  ...
  for (const M of mValues) {
    ...
    console.log(
      `  M=${M}: recall=${(recall / 50 * 100).toFixed(1)}%  search=${formatNum(1_000_000 / sl.medianUs)}/s  mem=${
        (index.memoryUsage() / (1024 * 1024)).toFixed(1)
      }MB`,
    )
  }
